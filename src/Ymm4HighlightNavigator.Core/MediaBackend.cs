...
public sealed class FfmpegBackend
{
    ...
    public async Task<MediaInfo> ProbeAsync(string path, CancellationToken token = default)
    {
        ...
        string json = await ChildProcess.CaptureAsync(FfprobePath, ["-v", "error", "-protocol_whitelist", "file,pipe", "-show_entries", "format=duration:stream=codec_type", "-of", "json", path], TimeSpan.FromSeconds(30), token).ConfigureAwait(false);
        using var doc = JsonDocument.Parse(json);
        var streams = doc.RootElement.GetProperty("streams").EnumerateArray().ToArray();
        if (!streams.Any(s => s.GetProperty("codec_type").GetString() == "video")) throw new InvalidDataException("No video stream.");
        string? duration = doc.RootElement.GetProperty("format").GetProperty("duration").GetString();
        if (!double.TryParse(duration, NumberStyles.Float, CultureInfo.InvariantCulture, out double seconds) || !double.IsFinite(seconds) || seconds <= 0)
            throw new InvalidDataException("Unknown recording duration.");
        return new(seconds, streams.Any(s => s.GetProperty("codec_type").GetString() == "audio"));
    }
        ...
        int fps = 2, IProgress<AnalysisProgress>? progress = null, CancellationToken token = default)
    {
        ...
        long initialSize = stamp.Length, initialWrite = stamp.LastWriteTimeUtc.Ticks;
        var info = await ProbeAsync(path, token).ConfigureAwait(false);
        double end = endSeconds ?? info.DurationSeconds;
        Guard.Range(startSeconds, end);
        if (end > info.DurationSeconds + .00001) throw new ArgumentOutOfRangeException(nameof(endSeconds), "Requested range exceeds the recording.");
        ...
        var common = new[] { "-nostdin", "-hide_banner", "-loglevel", "error", "-xerror", "-threads", "1", "-filter_threads", "1", "-protocol_whitelist", "file,pipe", "-ss", Number(startSeconds), "-i", path, "-t", Number(end - startSeconds) };
        var video = ImmutableArray.CreateBuilder<VideoFeature>();
        ...
        if (info.HasAudio)
        {
            ...
            await ChildProcess.StreamAsync(FfmpegPath, common.Concat(["-map", "0:a:0", "-vn", "-ac", "1", "-af", "aresample=8000:async=1:first_pts=0,apad", "-ar", "8000", "-f", "f32le", "pipe:1"]),
                async (stream, ct) =>
                {
                    byte[] block = new byte[4 * FeatureFormat.AudioHop];
                    long samples = 0;
                    while (true)
                    {
                        int used = 0, read;
        ...
        pack.Validate();
        progress?.Report(new("特徴データを検証", end, 1));
        return pack;
    }
    ...
    private static async Task<bool> ReadBlockAsync(Stream stream, Memory<byte> block, CancellationToken token)
    {
        ...
        while (used < block.Length)
        {
            ...
            if (read == 0)
            {
                ...
                throw new InvalidDataException("Truncated raw video frame.");
            }
            used += read;
        }
        return true;
...
public static class ChildProcess
{
        ...
        Func<Stream, CancellationToken, Task> consumer, TimeSpan timeout, CancellationToken token)
    {
        ...
        if (!process.Start()) throw new IOException("Could not start the analysis backend.");
        void Kill()
        {
        ...
        {
            Kill();
            if (!process.WaitForExit(5000)) throw new IOException("Backend did not terminate after cancellation.");
