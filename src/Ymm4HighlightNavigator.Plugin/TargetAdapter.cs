...
public sealed class TargetAdapter
{
    ...
    {
        var f = FileVersionInfo.GetVersionInfo(typeof(VideoItem).Assembly.Location);
        var actual = new Version(f.FileMajorPart, f.FileMinorPart, f.FileBuildPart, f.FilePrivatePart);
        if (actual != new Version(4, 56, 1, 0))
            throw new NotSupportedException($"この開発版のYMM4連携は4.56.1.0で検証しています。現在: {actual}");
    }
    ...
    private static TargetSnapshot Read(VideoItem item, int fps, Guid id)
    {
        ...
        if (Math.Abs(start.TotalSeconds - snapshot.SourceRange.Start) > 1e-6 || Math.Abs(end.TotalSeconds - snapshot.SourceRange.End) > 1e-6)
            throw new NotSupportedException("YMM4の時間変換が検証済みの定速モデルと一致しません。");
        return snapshot;
    }
    ...
    private sealed class HostMap
    {
        ...
        public static HostMap Read(VideoItem item)
        {
            var property = typeof(VideoItem).GetProperty("PlaybackRateMap", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
                ?? throw new MissingMemberException("このYMM4ではPlaybackRateMapを取得できません。");
            return new(property.GetValue(item) ?? throw new InvalidOperationException("PlaybackRateMapがありません。"));
        }
        public TimeSpan Forward(TimeSpan time, int length, int fps, TimeSpan offset, TimeSpan mediaDuration)
            => (TimeSpan)(forward.Invoke(value, [time, length, fps, offset, mediaDuration]) ?? throw new InvalidOperationException("Source timeがありません。"));
