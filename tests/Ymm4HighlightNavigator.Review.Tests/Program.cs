...
ReviewSet Saved(string id = "user.a", string name = "いつもの確認") => new(id, name, A().Normalize());
...
Test("duplicate-set-uses-new-id", () => { var s = Store(); var a = s.SaveNew("A", A(), 0); var b = s.SaveNew("B", a.Sets[0].Configuration, a.Revision); Assert(b.Sets.Length == 2 && b.Sets[0].Id != b.Sets[1].Id && b.Sets[0].Configuration.EquivalentTo(b.Sets[1].Configuration)); });
