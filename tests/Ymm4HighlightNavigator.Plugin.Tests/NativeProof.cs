...
internal static class Proof
{
        ...
        timer.Tick += async (_, _) =>
        {
            try
            {
                ...
                if (view?.DataContext is NavigatorModel model && active != null)
                {
                    ...
                    if (timeline != null) { timer.Stop(); await RunAsync(view, model, timeline); Write(true, null); return; }
                }
    ...
    {
        var pending = new Queue<DependencyObject>(); var seen = new HashSet<DependencyObject>(ReferenceEqualityComparer.Instance); pending.Enqueue(root);
    ...
    private static void TraceStartup(int ticks, bool opened, object? main, object? active)
    {
        ...
        if (active != null) lines.Add("ACTIVE_FIELDS " + string.Join(",", active.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic).Where(f => f.FieldType == typeof(Timeline)).Select(f => f.Name + ":" + f.FieldType.FullName)));
    ...
    private static bool OpenTool(object main)
    {
        ...
        bool Visit(object item, int depth)
        {
            ...
            var type = item.GetType(); string label = string.Join(" | ", new[] { "Header", "Title", "Name" }.Select(n => Text(Read(item, n))));
            ...
            { var value = Read(item, member); ownTool |= value is NavigatorModel || value is NavigatorPlugin; }
            if (menuTrace.Add(type.FullName + ":" + label)) File.AppendAllText(Path.Combine(output, "startup.txt"), "MENU " + type.FullName + ":" + label + "; members=" + string.Join(",", type.GetProperties().Select(p => p.Name + ":" + p.PropertyType.Name)) + "\n");
            if (ownTool)
            {
                foreach (string name in new[] { "ShowCommand", "OpenCommand", "ActivateCommand", "ShowToolCommand", "ShowWindowCommand", "ToggleVisibleCommand", "ToggleVisibilityCommand", "Command" })
                {
                    ...
                    var parameter = Read(item, "CommandParameter"); if (!command.CanExecute(parameter)) continue;
                    command.Execute(parameter); File.AppendAllText(Path.Combine(output, "startup.txt"), "OPENED " + name + "\n"); return true;
                }
                var visible = type.GetProperty("IsVisible");
                if (visible?.PropertyType == typeof(bool) && visible.SetMethod?.IsPublic == true)
                { visible.SetValue(item, true); File.AppendAllText(Path.Combine(output, "startup.txt"), "OPENED public IsVisible\n"); return true; }
            }
            foreach (var property in new[] { "Items", "Children", "MenuItems" })
                if (Read(item, property) is IEnumerable children) foreach (var child in children) if (child != null && Visit(child, depth + 1)) return true;
            return false;
        }
        if (Read(main, "ToolMenuItems") is not IEnumerable roots) return false;
        foreach (var root in roots) if (root != null && Visit(root, 0)) return true; return false;
    }
    private static async Task RunAsync(NavigatorView view, NavigatorModel model, Timeline timeline)
    {
        ...
        string beforeHash = Convert.ToHexString(SHA256.HashData(File.ReadAllBytes(media))); int fps = timeline.VideoInfo.FPS;
        ...
        string beforeState = Signature(); timeline.SelectedItems = ImmutableList.Create<IItem>(a, b); model.CaptureSelection();
        ...
        var adapter = new TargetAdapter(); adapter.Attach(timeline); adapter.CaptureSelection(); var first = adapter.Snapshots.Single(t => t.StartFrame == a.Frame);
        Check("native_map_fractional_projection", adapter.Project(first.Id, new(1.5173, 1.7)) == 153);
        Check("exclusive_end_no_jump", adapter.Project(first.Id, new(5, 5.1)) == null);
        var ids = model.Targets.Select(t => t.Id).ToArray(); timeline.SelectedItems = ImmutableList<IItem>.Empty;
        Check("selection_independent_targets", model.Targets.Select(t => t.Id).SequenceEqual(ids));
        bool rejected = false; try { model.CaptureSelection(); } catch (InvalidOperationException) { rejected = true; }
        Check("empty_capture_atomic", rejected && model.Targets.Select(t => t.Id).SequenceEqual(ids));
        int heartbeats = 0; var heartbeat = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(20) };
        heartbeat.Tick += (_, _) => heartbeats++; heartbeat.Start();
        try { await model.AnalyzeAsync(Ymm4FfmpegLocator.CreateBackend()); } finally { heartbeat.Stop(); }
        ...
        try
        {
            foreach (var p in model.Profiles) p.Enabled = false; await model.RequeryAsync();
            Check("all_profiles_off", model.Candidates.Count == 0 && model.CandidateSummary.Contains("ヒット計 0", StringComparison.Ordinal));
            model.Profiles[0].Enabled = true; await model.RequeryAsync(); Check("toggle_without_decoder", model.Candidates.Count > 0 && model.DecodedRangeCount == 1);
            model.Sensitivity = 1.5; await model.RequeryAsync(); Check("sensitivity_without_decoder", model.Candidates.Count > 0);
            model.Sensitivity = 1; foreach (var p in model.Profiles) p.Enabled = true; await model.RequeryAsync();
            await model.RequeryAsync(new CancellationToken(true)); Check("cancelled_query_not_old_success", model.Candidates.Count == 0 && !model.CandidateSummary.Contains("ヒット計", StringComparison.Ordinal));
            await model.RequeryAsync(); Check("query_recovers_without_redecode", model.Candidates.Count > 0 && model.DecodedRangeCount == 1);
        }
        ...
        Check("narrow_primary_controls_contained", new[] { "CaptureButton", "AnalyzeButton", "PreviousButton", "NextButton", "CandidateList", "LearningButton", "SensitivitySlider" }.All(Inside));
    ...
    {
        assertions.Add(new { id, passed }); File.AppendAllText(Path.Combine(output, "assertions.txt"), $"ASSERT {(passed ? "PASS" : "FAIL")} {id}\n");
