...
**現在: 教材動画から切り替わりフィルターを作り、試用・保存・候補巡回する初期実装が動作しています。完成品・一般配布版ではありません。** 設計はv0.4.2。実装済み範囲と検証根拠は [チェックポイント](docs/IMPLEMENTATION_STATUS.md) を参照してください。
...
- 実YMM4 Lite4.56.1.0で、PR #9のUI/UX基盤とPortable Dataを含む **111独立項目**の製品統合テストを実行しました。旧LocalAppDataからのLearning/Review移行、Portable優先、破損時no-fallback、保存先controlのnarrow layoutも含みます。
